...
exports.MarkupParser = void 0;
const errors_1 = require("../errors");
const ast_1 = require("../ast");
const expression_adapter_1 = require("./expression-adapter");
...
class MarkupParser {
     ...
     */
    inCondition;
    constructor(tokens, source, markupStart, markupEnd) {
        this.tokens = tokens;
        this.source = source;
        this.p = 0;
    ...
    atValueStart() {
        switch (this.peek().type) {
            case tokenizer_1.MarkupTokenType.String:
            case tokenizer_1.MarkupTokenType.Number:
            case tokenizer_1.MarkupTokenType.Id:
            case tokenizer_1.MarkupTokenType.OpenSquare:
            case tokenizer_1.MarkupTokenType.OpenRound:
                return true;
            case tokenizer_1.MarkupTokenType.Dash:
                return this.lax;
            default:
                return false;
        }
    ...
    openSquareContinuesLookup(prevEnd, openSquare) {
        if (!this.lax)
            return true;
    ...
    skipToNextPipe() {
        while (!this.isAtEnd() && !this.look(tokenizer_1.MarkupTokenType.Pipe)) {
            this.p += 1;
        }
    }
    consume(type) {
        ...
        if (token.type !== type) {
            throw new errors_1.LiquidHTMLASTParsingError(`Expected ${type} but found ${token.type} (${token.value})`, this.source, token.start, token.end);
        }
        this.p += 1;
        return token;
    }
    ...
    id(keyword) {
        const token = this.tokens[this.p];
        if (token.type !== tokenizer_1.MarkupTokenType.Id || token.value !== keyword)
            return false;
        this.p += 1;
        return true;
    }
    isAtEnd() {
        return this.tokens[this.p].type === tokenizer_1.MarkupTokenType.EndOfString;
    }
    ...
    remainingSource() {
        ...
        if (current.type === tokenizer_1.MarkupTokenType.EndOfString)
            return '';
        ...
        switch (token.type) {
            ...
            case tokenizer_1.MarkupTokenType.Number: {
                ...
                if (this.lax) {
                    ...
                    if (next && next.type === tokenizer_1.MarkupTokenType.Id && next.start === token.end) {
                        this.p += 2;
                        return {
            ...
            case tokenizer_1.MarkupTokenType.Id: {
                ...
                const next = this.tokens[this.p + 1];
                const followedByBracketLookup = next?.type === tokenizer_1.MarkupTokenType.OpenSquare &&
                    this.openSquareContinuesLookup(token.end, next);
                if (isLiteralKeyword(token.value) &&
                    !this.look(tokenizer_1.MarkupTokenType.Dot, 1) &&
                    ...
                    return {
                        ...
                        keyword: token.value,
                        value: ast_1.LiquidLiteralValues[token.value],
                        position: { start: token.start, end: token.end },
                        source: this.source,
                    };
                }
            ...
            case tokenizer_1.MarkupTokenType.OpenSquare:
                return this.variableLookup();
            case tokenizer_1.MarkupTokenType.Dot:
                ...
                if (this.lax) {
                    const next = this.tokens[this.p + 1];
                    if (next && next.type === tokenizer_1.MarkupTokenType.Number && next.start === token.end) {
                        ...
                        if (!next.value.includes('.')) {
                            ...
                            while (after &&
                                ...
                                (after.type === tokenizer_1.MarkupTokenType.Id ||
                                    after.type === tokenizer_1.MarkupTokenType.Dash ||
                                    after.type === tokenizer_1.MarkupTokenType.Number)) {
                                segment += after.value;
                                end = after.end;
                                consumed += 1;
                ...
                throw new errors_1.LiquidHTMLASTParsingError(`Expected expression but found ${token.type} (${token.value})`, this.source, token.start, token.end);
            case tokenizer_1.MarkupTokenType.OpenRound: {
                const openToken = this.consume(tokenizer_1.MarkupTokenType.OpenRound);
                ...
                if (this.lax && !this.parenContainsRange()) {
                    const raw = this.consumeRawUntil(tokenizer_1.MarkupTokenType.CloseRound);
                    this.consumeOptional(tokenizer_1.MarkupTokenType.CloseRound);
                    this.discardRemaining();
                    return {
                        type: types_1.NodeTypes.VariableLookup,
                        name: raw,
                ...
                this.consume(tokenizer_1.MarkupTokenType.DotDot);
                // Lax: Ruby's RANGES_REGEX tolerates extra dots, e.g. `(1...5)` and
                // `(1.....5)` both parse as the range 1..5. Skip any surplus Dot/DotDot.
                if (this.lax) {
                    while (this.look(tokenizer_1.MarkupTokenType.Dot) || this.look(tokenizer_1.MarkupTokenType.DotDot)) {
                        this.p += 1;
                    }
                }
                ...
                const closeToken = this.lax
                    ? this.consumeOptional(tokenizer_1.MarkupTokenType.CloseRound)
                    : this.consume(tokenizer_1.MarkupTokenType.CloseRound);
    ...
    parenContainsRange() {
        ...
        for (let i = this.p; i < this.tokens.length; i++) {
            ...
            if (t.type === tokenizer_1.MarkupTokenType.EndOfString)
                break;
            if (t.type === tokenizer_1.MarkupTokenType.OpenRound)
                depth += 1;
            else if (t.type === tokenizer_1.MarkupTokenType.CloseRound) {
                if (depth === 0)
                    break;
                depth -= 1;
            }
            else if (t.type === tokenizer_1.MarkupTokenType.DotDot && depth === 0) {
                return true;
            }
        }
        return false;
    ...
    variableLookup() {
        ...
        if (current.type === tokenizer_1.MarkupTokenType.Id) {
            const token = this.consume(tokenizer_1.MarkupTokenType.Id);
            name = token.value;
            startPos = token.start;
        }
        else if (current.type === tokenizer_1.MarkupTokenType.OpenSquare) {
            name = null;
            startPos = current.start;
        }
        else if (this.lax &&
            current.type === tokenizer_1.MarkupTokenType.Number &&
            ...
            throw new errors_1.LiquidHTMLASTParsingError(`Expected identifier or [ but found ${current.type} (${current.value})`, this.source, current.start, current.end);
        }
        const lookups = [];
        ...
        let laxSegment = null;
        while (!this.isAtEnd()) {
            if (this.consumeOptional(tokenizer_1.MarkupTokenType.Dot)) {
                const prop = this.consume(tokenizer_1.MarkupTokenType.Id);
                lookups.push({
                    type: types_1.NodeTypes.String,
                    value: prop.value,
            ...
            else if (this.look(tokenizer_1.MarkupTokenType.OpenSquare) &&
                this.openSquareContinuesLookup(endPos, this.peek())) {
                this.consume(tokenizer_1.MarkupTokenType.OpenSquare);
                const inner = this.valueExpression();
                // Lax: a missing closing bracket (`['foo'`, `[['BIG']]`) is tolerated.
                const close = this.lax
                    ? this.consumeOptional(tokenizer_1.MarkupTokenType.CloseSquare)
                    : this.consume(tokenizer_1.MarkupTokenType.CloseSquare);
                lookups.push(inner);
                ...
                if (this.lax &&
                    !close &&
                    !this.look(tokenizer_1.MarkupTokenType.Dot) &&
                    !this.look(tokenizer_1.MarkupTokenType.OpenSquare)) {
                    break;
                }
            }
            else if (this.lax &&
        ...
        if (this.lax &&
            name === null &&
            lookups.length === 1 &&
            lookups[0].type === types_1.NodeTypes.VariableLookup &&
            lookups[0].name === null &&
     ...
     * unchanged.
     */
    tryConsumeLaxLookupSeparator(prevEnd) {
        let i = this.p;
        ...
        while (i < this.tokens.length) {
            ...
            if (tok.type === tokenizer_1.MarkupTokenType.EndOfString)
                return null;
            if (tok.start !== expectedStart)
                return null; // whitespace gap → fragment ended
            ...
            const isSymbolicOperator = (tok.type === tokenizer_1.MarkupTokenType.Equality || tok.type === tokenizer_1.MarkupTokenType.Comparison) &&
                /^[=<>!]+$/.test(tok.value);
        ...
        if (i === this.p)
            return null; // no non-word punctuation was skipped; nothing to recover
        ...
        if (!match || match.index !== expectedStart)
            return null;
        const value = match[0];
        ...
        while (j < this.tokens.length &&
            this.tokens[j].type !== tokenizer_1.MarkupTokenType.EndOfString &&
     ...
     */
    laxLookupSeparatorFollows(prevEnd) {
        ...
        while (i < this.tokens.length) {
            ...
            if (tok.type === tokenizer_1.MarkupTokenType.EndOfString)
                break;
            if (tok.start !== expectedStart)
                break; // whitespace gap → fragment ended
            const isSymbolicOperator = (tok.type === tokenizer_1.MarkupTokenType.Equality || tok.type === tokenizer_1.MarkupTokenType.Comparison) &&
                /^[=<>!]+$/.test(tok.value);
            if (!isSymbolicOperator)
                break;
        ...
        if (i === startIndex)
            return false; // no symbolic-operator run skipped
    ...
    comparison() {
        ...
        if (this.lax && this.inCondition)
            this.skipLaxConditionParens();
        ...
        if (this.lax && this.inCondition && token.type !== tokenizer_1.MarkupTokenType.Logical) {
            ...
            if (opRun &&
                ...
                token.type !== tokenizer_1.MarkupTokenType.Comparison &&
                token.type !== tokenizer_1.MarkupTokenType.Equality) {
                return this.mergedOperatorComparison(left, opRun);
            }
        }
        if (token.type !== tokenizer_1.MarkupTokenType.Comparison && token.type !== tokenizer_1.MarkupTokenType.Equality) {
        ...
        if (!isComparisonOp(token.value)) {
            ...
            if (this.lax)
                return left;
            throw new errors_1.LiquidHTMLASTParsingError(`Unknown operator ${token.value}`, this.source, token.start, token.end);
        }
        this.p += 1;
    ...
    laxConditionOperatorRun(afterLeft) {
        ...
        let start = afterLeft;
        while (start < this.markupEnd && /\s/.test(this.source[start]))
            start += 1;
     ...
     *  `lax && inCondition`. */
    unknownOperatorComparison(left, op) {
        while (this.peek().type !== tokenizer_1.MarkupTokenType.EndOfString && this.peek().start < op.end) {
            this.p += 1;
        }
        let right = left;
        if (this.atValueStart()) {
            ...
            try {
                ...
                this.p = savedP;
                while (this.peek().type !== tokenizer_1.MarkupTokenType.EndOfString &&
                    this.peek().type !== tokenizer_1.MarkupTokenType.Logical) {
                    this.p += 1;
                }
                right = left;
            }
    ...
    mergedOperatorComparison(left, op) {
        ...
        const restParser = new MarkupParser((0, tokenizer_1.tokenizeMarkup)(rest, op.end), this.source, op.end, this.markupEnd).enableLax();
    ...
    skipLaxConditionParens() {
        while (this.look(tokenizer_1.MarkupTokenType.OpenRound) && !this.parenAtCursorContainsRange()) {
            this.p += 1;
        }
    }
    /** Lax helper: like parenContainsRange but assumes the cursor is ON the
     *  OpenRound; scans its body for a depth-0 `..`. */
    parenAtCursorContainsRange() {
        let depth = 0;
        for (let i = this.p; i < this.tokens.length; i++) {
            const t = this.tokens[i];
            if (t.type === tokenizer_1.MarkupTokenType.EndOfString)
            ...
            else if (t.type === tokenizer_1.MarkupTokenType.DotDot && depth === 1) {
                return true;
            }
        }
        return false;
    ...
    logicalExpr() {
        ...
        const token = this.peek();
        if (token.type !== tokenizer_1.MarkupTokenType.Logical) {
            return left;
        }
        if (!isLogicalOp(token.value)) {
            ...
            if (this.lax)
                return left;
            throw new errors_1.LiquidHTMLASTParsingError(`Unknown logical operator: ${token.value}`, this.source, token.start, token.end);
        }
    ...
    conditionalExpression() {
        ...
        let result;
        try {
            result = (0, expression_adapter_1.adaptConditional)(this.logicalExpr());
        }
        finally {
        ...
        if (result.type === types_1.NodeTypes.LogicalExpression) {
    ...
    // expression := logical
    expression() {
        return (0, expression_adapter_1.adaptComplex)(this.logicalExpr());
    }
    ...
    liquidVariable() {
        ...
        const tokenEnd = filterList.length > 0 ? filterList[filterList.length - 1].position.end : expr.position.end;
    ...
    // filters := ("|" filter)*
    filters(previousEnd = 0) {
        const result = [];
        while (this.consumeOptional(tokenizer_1.MarkupTokenType.Pipe)) {
            // Lax: a `|` not followed by a filter name (e.g. trailing `|`) is dropped.
            if ((this.lax || this.tolerant) && !this.look(tokenizer_1.MarkupTokenType.Id)) {
                this.skipToNextPipe();
                continue;
            }
            ...
            if (this.lax && !this.isAtEnd() && !this.look(tokenizer_1.MarkupTokenType.Pipe)) {
                this.skipToNextPipe();
            }
        }
        return result;
    ...
    // filter := id (":" arguments)?
    filter(previousEnd) {
        const nameToken = this.consume(tokenizer_1.MarkupTokenType.Id);
        let args = [];
        let end = nameToken.end;
        if (this.consumeOptional(tokenizer_1.MarkupTokenType.Colon)) {
    ...
    argument() {
        if (this.look(tokenizer_1.MarkupTokenType.Id) && this.lookaheadForNamedArgument()) {
    ...
    lookaheadForNamedArgument() {
        let ahead = 1;
        while (this.look(tokenizer_1.MarkupTokenType.Dot, ahead) && this.look(tokenizer_1.MarkupTokenType.Id, ahead + 1)) {
            ahead += 2;
        }
        return this.look(tokenizer_1.MarkupTokenType.Colon, ahead);
    }
    // namedArgument := id ("." id)* ":" valueExpression
    namedArgument() {
        const nameToken = this.consume(tokenizer_1.MarkupTokenType.Id);
        let name = nameToken.value;
        while (this.consumeOptional(tokenizer_1.MarkupTokenType.Dot)) {
            const segment = this.consume(tokenizer_1.MarkupTokenType.Id);
            name += '.' + segment.value;
        }
        this.consume(tokenizer_1.MarkupTokenType.Colon);
    ...
    arguments() {
        ...
        while (this.consumeOptional(tokenizer_1.MarkupTokenType.Comma)) {
    ...
    namedArguments() {
        ...
        while (this.consumeOptional(tokenizer_1.MarkupTokenType.Comma)) {
            if (!this.look(tokenizer_1.MarkupTokenType.Id))
     ...
     */
    blockValueExpression() {
        if (this.look(tokenizer_1.MarkupTokenType.OpenSquare)) {
     ...
     */
    assertNotNestedArray() {
        if (this.look(tokenizer_1.MarkupTokenType.OpenSquare)) {
            const token = this.peek();
            throw new errors_1.LiquidHTMLASTParsingError('nested arrays are not supported in block array literal', this.source, token.start, token.end);
        }
    }
    /*
     ...
     */
    blockArrayLiteral() {
        const openToken = this.consume(tokenizer_1.MarkupTokenType.OpenSquare);
        const elements = [];
        while (!this.look(tokenizer_1.MarkupTokenType.CloseSquare)) {
            this.assertNotNestedArray();
            elements.push(this.valueExpression());
            if (!this.look(tokenizer_1.MarkupTokenType.CloseSquare)) {
                this.consume(tokenizer_1.MarkupTokenType.Comma);
            }
        }
        const closeToken = this.consume(tokenizer_1.MarkupTokenType.CloseSquare);
        return {
    ...
    computeLastTokenEnd() {
        for (let i = this.tokens.length - 1; i >= 0; i--) {
            if (this.tokens[i].type !== tokenizer_1.MarkupTokenType.EndOfString) {
...
function extendLogicalEnd(node, end) {
    if (node.type !== types_1.NodeTypes.LogicalExpression)
        return;
    node.position = { ...node.position, end };
    extendLogicalEnd(node.right, end);
}
function isComparisonOp(s) {
    return expression_adapter_1.EQUALITY_OPERATORS.has(s) || expression_adapter_1.COMPARISON_OPERATORS.has(s);
}
function isLogicalOp(s) {
    return expression_adapter_1.LOGICAL_OPERATORS.has(s);
}
function isLiteralKeyword(s) {
    return s in ast_1.LiquidLiteralValues;
}
