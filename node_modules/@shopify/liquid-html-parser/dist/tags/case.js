...
exports.whenBranchParse = whenBranchParse;
...
function whenBranchParse(_name, markup) {
    const values = [markup.valueExpression()];
    while (true) {
        if (markup.consumeOptional(tokenizer_1.MarkupTokenType.Comma)) {
            ...
            if (markup.isLax() && !markup.atValueStart())
                break;
            values.push(markup.valueExpression());
            continue;
        }
        const token = markup.peek();
        if (token.type === tokenizer_1.MarkupTokenType.Logical && token.value === 'or') {
            markup.consume(tokenizer_1.MarkupTokenType.Logical);
            if (markup.isLax() && !markup.atValueStart())
                break;
            values.push(markup.valueExpression());
            continue;
    ...
    if (trailingColon.type === tokenizer_1.MarkupTokenType.Colon &&
        markup.look(tokenizer_1.MarkupTokenType.EndOfString, 1) &&
        lastValue.type === types_1.NodeTypes.String &&
        lastValue.position.end === trailingColon.start) {
        markup.consume(tokenizer_1.MarkupTokenType.Colon);
    }
