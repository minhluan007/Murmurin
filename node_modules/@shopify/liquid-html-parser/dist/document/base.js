...
class ParserBase {
    ...
    consume(type) {
        ...
        if (token.type !== type) {
            throw new errors_1.LiquidHTMLASTParsingError(`Expected ${type} but got ${token.type}`, this.source, token.start, token.end);
        }
        this.p++;
        return token;
    }
    ...
    isAtEnd() {
        return this.tokens[this.p].type === tokenizer_1.TokenType.EndOfInput;
    }
    getSource() {
        return this.source;
    }
     ...
     */
    seekToSourceOffset(sourceOffset) {
        let i = 0;
        while (i < this.tokens.length && this.tokens[i].start < sourceOffset)
            i++;
        this.p = i;
    }
    /**
     ...
     */
    openQuoteCharBefore(prefix) {
        let openQuote;
        for (const token of prefix) {
            if (token.type === tokenizer_1.TokenType.HtmlQuoteOpen)
                openQuote = token;
            else if (token.type === tokenizer_1.TokenType.HtmlQuoteClose)
                openQuote = undefined;
        }
     ...
     */
    insideOpenHtmlTag(prefix) {
        let open = false;
        for (const token of prefix) {
            if (token.type === tokenizer_1.TokenType.HtmlTagOpen ||
                token.type === tokenizer_1.TokenType.HtmlCloseTagOpen ||
                token.type === tokenizer_1.TokenType.HtmlDoctypeOpen) {
                open = true;
            }
            else if (token.type === tokenizer_1.TokenType.HtmlTagClose || token.type === tokenizer_1.TokenType.HtmlSelfClose) {
                open = false;
            }
        }
        return open;
    }
    resliceTokensFrom(sourceOffset) {
        const straddles = this.tokens.some((token) => token.start < sourceOffset && token.end > sourceOffset);
        if (straddles) {
            ...
            const rebased = (0, tokenizer_1.tokenize)(this.source.slice(sourceOffset), {
