...
function printNamedLiquidBlockStart(path, _options, print, args, whitespaceStart, whitespaceEnd) {
    ...
    switch (node.name) {
         ...
         */
        case liquid_html_parser_1.NamedTags.block:
        case liquid_html_parser_1.NamedTags.partial: {
            return tag(' ');
        }
         ...
         */
        case liquid_html_parser_1.NamedTags.ifchanged: {
...
function printLiquidBlockStart(path, options, print, args = {}) {
     ...
     * printer ever saw it.
     */
    const printedMarkup = markup && !liquid_html_parser_1.TAGS_WITHOUT_MARKUP.includes(node.name) ? ` ${markup}` : '';
...
function printLiquidTag(path, options, print, args) {
     ...
     */
    if ((0, liquid_html_parser_1.isBranchedTag)(node) || node.name === 'tablerow') {
...
function printLiquidRawTag(path, options, print, { isLiquidStatement }) {
    ...
    const blockStart = isLiquidStatement
        ...
        : group([
             ...
             * markup as before.
             */
            node.markup && !liquid_html_parser_1.TAGS_WITHOUT_MARKUP.includes(node.name) ? `${node.markup} ` : '',
            node.whitespaceEnd,
            '%}',
        ]);
