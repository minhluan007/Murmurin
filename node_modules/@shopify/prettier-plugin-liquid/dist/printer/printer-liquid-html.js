...
function printNode(path, options, print, args = {}) {
    const node = path.getValue();
    if (node.type === 'BlockArrayLiteral') {
        return [
            '[',
    ...
    switch (node.type) {
         ...
         */
        case liquid_html_parser_1.NodeTypes.BlockMarkup:
        case liquid_html_parser_1.NodeTypes.SectionMarkup: {
            const doc = [path.call((p) => print(p), 'name')];
            if (node.args.length > 0) {
                doc.push(',', line, join([',', line], path.map((p) => print(p), 'args')));
            }
            return doc;
        ...
        case liquid_html_parser_1.NodeTypes.LiquidErrorNode: {
            return node.source.slice(node.position.start, node.position.end);
        }
        default: {
